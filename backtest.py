...
class BacktestSimulator:
    ...
    def _params_snapshot(self) -> dict:
        """
        Config 필드명과 1:1로 일치하는 키를 사용합니다.
        GRID_PARAMS 키 및 WFAScheduler 자동 적용(cfg.{key} = value)과 호환됩니다.
        기존 축약 키(take_profit, rsi_min 등)는 제거되었습니다.
        """
        cfg = self.cfg
        snapshot = {
            ...
            "pullback_max_below_ma_pct": cfg.pullback_max_below_ma_pct,
            "min_entry_score":           getattr(cfg, "min_entry_score", 60.0),
        }
        # 선택적 파라미터 — 존재하는 경우만 포함
        optional_fields = [
            "use_trailing_stop",
        ...
        for field in optional_fields:
            if hasattr(cfg, field):
                snapshot[field] = getattr(cfg, field)
        return snapshot
...
def run_grid_search(
    ...
    # ── best 결과를 JSON으로 저장 (WFAScheduler 자동 적용용) ──────────────────
    # run_grid_search()는 grid_csv만 저장했는데, WFAScheduler._save_best_params()는
    # result_*.json 파일을 찾으므로, 1등 결과를 반드시 JSON으로도 저장해야 합니다.
    if results:
        best = results[0]
        if best.n_trades >= 5:
            best_json_path = f"{out_dir}/result_{market.replace('-', '_')}_best.json"
            save_result_json(best, best_json_path)
            print(f"[save] 최적 파라미터 JSON → {best_json_path}")
        else:
            print("[warn] 거래 횟수가 5회 미만이라 best JSON 저장을 건너뜁니다.")
