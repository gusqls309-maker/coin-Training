            ...
            for market in eligible_markets:
                ...
                if prev_base_total > 0 and snap["base_total"] <= 0:
                    ...
                    if exit_reason == "take_profit":
                        state_store.reset_market_consecutive_loss(market)

                    is_loss = exit_reason == "stop_loss"

                    # ── 전역 서킷브레이커 카운터 갱신 ────────────────────────
                    if is_loss and getattr(cfg, "circuit_breaker_enabled", False):
                    ...
                    # ── 종목별 연속 손실 / 빠른 손실 차단 (기능 A, B) ────────
                    if is_loss and (
                        getattr(cfg, "per_market_loss_block_enabled", False)
                        or getattr(cfg, "quick_loss_block_enabled", False)
                    ):
                        blocked, block_reason = state_store.record_market_loss(
                            market=market,
                            max_consecutive=cfg.per_market_max_consecutive_loss,
                            cooldown_sec=cfg.per_market_block_sec,
                            hold_sec=state_store.hold_seconds(market),   # 아직 clear 전
                            quick_loss_threshold_sec=cfg.quick_loss_threshold_sec,
                            quick_loss_max=cfg.quick_loss_max_count,
                            quick_loss_window_sec=cfg.quick_loss_window_sec,
                        )
                        if blocked:
                            logging.warning(
                                "🚫 종목별 진입 차단 발동 | market=%s | %s",
                                market, block_reason
                            )
                ...
                # ── 종목별 진입 차단 여부 확인 (기능 A, B) ───────────────────
                market_blocked = state_store.is_market_blocked(market)
                if market_blocked:
                    remaining_block = state_store.market_block_remaining_sec(market)
                    logging.debug(
                        "종목별 진입 차단 중 | market=%s | 해제까지 %.0f분",
                        market, remaining_block / 60,
                    )

                buy_ok = (
                    score >= cfg.min_entry_score
                    and strat.get("htf_trend_up", False)
                    ...
                    and not market_blocked                # 종목별 차단 적용
                ...
                analyses[market] = {
                    ...
                    "sell_reason_class": classify_exit_reason(sell_reason) if sell_ok else "",
                    ...
                    if manage_sell:
                        ...
                        if precheck_ok and sell_volume > 0:
                            ...
                            place_or_reprice_limit_sell(
                                ...
                                # ── 기능 C: 구조화된 청산 정보 전달 ──────────
                                exit_reason=a.get("sell_reason_class"),
                                sell_reason=a.get("sell_reason", ""),
                                entry_price=a["snap"].get("avg_buy_price"),
                                net_pnl_pct=a.get("net_pnl_pct"),
