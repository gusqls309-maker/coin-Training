...
class Config:
    ...
    # ── 종목별 연속 손실 차단 (기능 A) ──────────────────────────────────────
    # 같은 종목에서 연속으로 N회 손실 발생 시 해당 종목 신규 진입을 차단합니다.
    per_market_loss_block_enabled: bool = parse_bool(os.getenv("PER_MARKET_LOSS_BLOCK_ENABLED"), True)
    per_market_max_consecutive_loss: int = int(os.getenv("PER_MARKET_MAX_CONSECUTIVE_LOSS", "2"))
    per_market_block_sec: int = int(os.getenv("PER_MARKET_BLOCK_SEC", "21600"))  # 기본 6시간

    # ── 종목별 빠른 손실 차단 (기능 B) ──────────────────────────────────────
    # 짧은 시간(quick_loss_threshold_sec) 내 청산된 손실이 N회 이상 발생하면 차단합니다.
    quick_loss_block_enabled: bool = parse_bool(os.getenv("QUICK_LOSS_BLOCK_ENABLED"), True)
    quick_loss_threshold_sec: float = float(os.getenv("QUICK_LOSS_THRESHOLD_SEC", "900"))   # 15분
    quick_loss_max_count: int = int(os.getenv("QUICK_LOSS_MAX_COUNT", "2"))
    quick_loss_window_sec: int = int(os.getenv("QUICK_LOSS_WINDOW_SEC", "3600"))   # 1시간 이내
